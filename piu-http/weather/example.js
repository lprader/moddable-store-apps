...
const iconTexture = new Texture("icons.png");
...
const OpenSans24 = new Style({ font: "24px Open Sans", color: WHITE });
...
class MainColBehavior extends Behavior {
	...
	onFinished(column) {
		if (this.transitioningIn)
			...
			column.container.defer("onAddNextScreen");
	}
...
const MainCol = Column.template($ => ({
	...
	contents:[
		new AnimatedText({ hue: 0, string: $.city, style: OpenSans24, }, { name: "city", top: 13, height: 38, left: 240,  }),
		...
		new AnimatedText({ hue: 270, string: $.condition, style: OpenSans24, }, { name: "condition", top: 1, height: 38, }),	
	],
...
class WeatherAppBehavior extends Behavior {
	...
	onDisplaying(container) {
		this.zipIndex = 0;
		this.getWeatherData(container, zips[this.zipIndex]); 
	}
	onFinished(container) {
		if (++this.zipIndex >= zips.length) this.zipIndex = 0;
		let zip = zips[this.zipIndex];
		this.getWeatherData(container, zip);     
	}
	getWeatherData(container, zip) {
		...
		request.callback = (message, value) => {
			if (Request.responseComplete == message) {
				...
				this.data.icon = icon;
				container.first.delegate("onTransitionOut");
				container.time = 0;
				container.start();
			}
			else if (Request.error == message) {
				container.first.string = "HTTP request failed";
			}
		}
	}
	onAddNextScreen(container) {
		container.empty();
		container.add(new MainCol(this.data));
	}
}

const WeatherForecast = Container.template($ => ({
	left: 0, right: 0, top: 0, bottom: 0, skin: blackSkin, 
	contents: [
		Text($, {
			left: 20, right: 20, top: 100,
			skin: blackSkin, style: OpenSans24, string: "Loading...",
			Behavior: class extends Behavior {
				onTransitionOut(text) {
					text.container.defer("onAddNextScreen");
				}
			}
		}),
	],
...
application.add(new WeatherForecast({}));
